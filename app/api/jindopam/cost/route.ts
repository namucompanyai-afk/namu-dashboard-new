...
export async function POST(req: Request) {
  try {
    ...
    // ── K열(최종 공급가) 행별 수식 전환 (수동 1회) ─────────────────────────
    // ARRAYFORMULA는 사용자가 시트를 정렬하면 깨지므로, C열에 품목이 있는 모든 행에
    // 행별 K 수식을 기록한다(각 행이 스스로 계산 → 정렬 안전). K12:K는 먼저 클리어.
    if (action === 'init16') {
      ...
      // 품목 있는 행만 K 수식, 빈 행은 빈값 유지
      let written = 0
      const kvals = cCol.slice(0, lastIdx + 1).map((row, i) => {
        const r = 12 + i
        if (String(row?.[0] ?? '').trim() === '') return ['']
        written++
        return [supplyFormulas(r)[5]] // K = 최종 공급가 행별 수식
      })
      await sheets.spreadsheets.values.update({
        spreadsheetId: SHEET_ID,
        range: `${quote(COST_TAB)}!K12:K${12 + lastIdx}`,
        valueInputOption: 'USER_ENTERED',
        requestBody: { values: kvals },
      })
    ...
    if (action === 'create') {
      ...
      const data: { range: string; values: any[][] }[] = [
        ...
        // F~K: 비용분해·최종공급가 수식 (K도 행별 → 정렬해도 안전)
        {
