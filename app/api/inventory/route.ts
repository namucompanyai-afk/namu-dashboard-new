...
export async function GET(request: Request) {
  ...
  try {
    const saved = await getData("inventory");
    if (!saved) return NextResponse.json({ products: [], uploadDate: null });
    return NextResponse.json({ products: saved.products || [], uploadDate: saved.uploadDate || null });
  } catch (err) {
    return NextResponse.json({ error: String(err) }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    await saveData("inventory", { products, uploadDate: new Date().toISOString() });
...
async function sendEmail(dateStr: string, restockItems: any[], urgentItems: any[], normalItems: any[], elapsedDays: number) {
  ...
  <div style="padding:24px 32px">
    <table style="width:100%;border-collapse:collapse;font-size:13px">
      <thead><tr style="background:#f9fafb">
        ...
        <th style="padding:8px 12px;text-align:left;color:#6b7280;font-size:11px">재고변화</th>
